...
(function(){
        ...
        if (config.requiresTerms) {
          if (this.termsCheckbox.checked) {
            ...
            gtag('event', 'check_out', {
                'send_to': 'AW-10944819516/Zz8gCMOWl80DELzi8uIo',
                'url': window.location
            });
          } else {
  ...
  theme.AjaxProduct = (function() {
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      addItemFromForm: function(evt, callback){
        ...
        var i = 1;
        while (data.includes("properties%5BTitle%5D"))
        {
          data = data.replace("properties%5BTitle%5D", "properties%5B" + i + "%5D");
          i ++;
        }
      ...
      success: function(product) {
        ...
        gtag('event', 'add_to_cart', {
            'send_to': 'AW-10944819516/Zz8gCMOWl80DELzi8uIo',
            'item': [{ 'item_id': product.id, 'item_name': product.title, 'quantity': product.quantity }],
            'url': window.location
        });
      },
  ...
  theme.Slideshow = (function() {
    ...
    function slideshow(el, args) {
      ...
      console.log("before flickity: " + Date.now());
      ...
      console.log("after flickity: " + Date.now());
...
function  customize_add_to_cart(addToCart,variant_id) {
      var form=$("#form_"+variant_id);
   
        // Loading indicator on add to cart button
        if(!$(addToCart).hasClass("btn--loading")){
           $(addToCart).addClass('btn--loading');
        }
        var data = {items:[{id: $("#id"+variant_id).val(), quantity: $("#quantity"+variant_id).val()}]};

         $.post(theme.routes.cartAdd,data,function(r_data){
            var product = r_data;
            customize_add_to_cart_success(product,addToCart);
         },'json').fail(function() {
            //customize_add_to_cart_error(r_data);
          }).always(function() {
             if($(addToCart).hasClass("btn--loading")){
...
function customize_add_to_cart_success(product,addToCart){
...
function customize_add_to_cart_error(error){
      if (!error.description) {
          console.warn(error);
          return;
        }
...
function refresh_free_gift() {
  console.log("refresh_free_gift");
  var free_gift_over_price_10000 = 0;
  var variant_id='41123879649331';
    var GetParams = {
      type: 'GET',
      url: '/cart.js',
      data: {  },
      ...
      success: function(cart) { 
        ...
        if( (totalQty-qtyInTheCart)>=5){
          cartChange={id:variant_id,quantity:1 };
          cartAdd=[{id:variant_id,quantity:1 }];
           if(qtyInTheCart>1){
            var params = {
              type: 'POST',
              url: '/cart/change.js',
              ...
              success: function(stuff) { 
                console.log("update gift to cart success");
          ...
          else
          {
            var params = {
              ...
              dataType: 'json',
              success: function(stuff) { 
                console.log("add gift to cart success");
                document.dispatchEvent(new CustomEvent('cart:build', {
                  }));
              }
            };
         ...
         cartChange={id:variant_id,quantity:0 };
          console.log("delete gift 1");
          if(qtyInTheCart>=1){
            console.log("delete gift 2");
            var params = {
              type: 'POST',
              url: '/cart/change.js',
              data: cartChange ,
              dataType: 'json',
              success: function(stuff) { 
                console.log("delete gift to cart success");
                document.dispatchEvent(new CustomEvent('cart:build', {
                }));
              }
          };
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let targetVariantIds = ["41127652261939", "41127652294707"]; // VALUE GIFTBOX 变体 ID

    let buttons = document.querySelectorAll("button[data-add-to-cart]");
    buttons.forEach(button => {
        let form = button.closest("form");
        if (form && form.querySelector("input[name='id']")) {
            let productId = form.querySelector("input[name='id']").value;
            
            if (targetVariantIds.includes(productId)) {
                let span = button.querySelector("span[data-add-to-cart-text]");
                
                let productCard = button.closest(".grid__item");
                let productUrl = productCard ? productCard.querySelector("a.grid-product__link")?.getAttribute("href") : null;

                if (productUrl) {
                    let newButton = document.createElement("a");
                    newButton.href = productUrl; 
                    newButton.className = button.className + " btn btn--full add-to-cart"; 
                    newButton.style.display = "inline-block"; 
                    newButton.style.textAlign = "center"; 
                    newButton.style.padding = "12px 24px"; 
                    newButton.style.backgroundColor = window.getComputedStyle(button).backgroundColor; 
                    newButton.style.color = window.getComputedStyle(button).color; 
                    newButton.style.borderRadius = window.getComputedStyle(button).borderRadius; 
                    newButton.style.fontSize = window.getComputedStyle(button).fontSize; 
                    newButton.style.fontWeight = window.getComputedStyle(button).fontWeight; 
                    newButton.innerHTML = "<span>Pick Your Styles</span>";
